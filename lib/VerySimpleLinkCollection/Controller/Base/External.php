 ...
 */
class VerySimpleLinkCollection_Controller_Base_External extends Zikula_AbstractController
{
    /**
    ...
    public function display(array $args = array())
    {
        ...
        $idFields = ModUtil::apiFunc('VerySimpleLinkCollection', 'selection', 'getIdFields', array('ot' => $objectType));
    ...
    {
        PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('VerySimpleLinkCollection'));
    
        $getData = $this->request->query;
        $controllerHelper = new VerySimpleLinkCollection_Util_Controller($this->serviceManager);
    
        $objectType = $getData->filter('objectType', 'link', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'external', 'action' => 'finder');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controller', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerType', $utilArgs);
        }
    
        $this->throwForbiddenUnless(SecurityUtil::checkPermission('VerySimpleLinkCollection:' . ucwords($objectType) . ':', '::', ACCESS_COMMENT), LogUtil::getErrorMsgPermission());
        ...
        $categoryIds = ModUtil::apiFunc('VerySimpleLinkCollection', 'category', 'retrieveCategoriesFromRequest', array('ot' => $objectType, 'source' => 'GET'));
