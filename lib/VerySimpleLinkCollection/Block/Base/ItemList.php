 ...
 */
class VerySimpleLinkCollection_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('VerySimpleLinkCollection:ItemListBlock:', 'Block title::');
    
        $this->categorisableObjectTypes = array('link');
    }
    ...
    public function info()
    {
        ...
        if (!ModUtil::available('VerySimpleLinkCollection')) {
            $requirementMessage .= $this->__('Notice: This block will not be displayed until you activate the VerySimpleLinkCollection module.');
        }
    
        return array('module'          => 'VerySimpleLinkCollection',
                     'text_type'       => $this->__('VerySimpleLinkCollection list view'),
                     'text_type_long'  => $this->__('Display list of VerySimpleLinkCollection objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!SecurityUtil::checkPermission('VerySimpleLinkCollection:ItemListBlock:', "$blockinfo[title]::", ACCESS_OVERVIEW)) {
            return false;
        }
        ...
        if (!isset($vars['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('VerySimpleLinkCollection', 'category', 'getPrimaryProperty', array('ot' => $vars['objectType']));
        ...
        ModUtil::initOOModule('VerySimpleLinkCollection');
        ...
        $entityClass = 'VerySimpleLinkCollection_Entity_' . ucwords($objectType);
        $entityManager = $this->serviceManager->getService('doctrine.entitymanager');
        $repository = $entityManager->getRepository($entityClass);
    
        $this->view->setCaching(Zikula_View::CACHE_ENABLED);
        // set cache id
        $component = 'VerySimpleLinkCollection:' . ucwords($objectType) . ':';
        $instance = '::';
        ...
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('VerySimpleLinkCollection', 'category', 'getAllProperties', array('ot' => $objectType));
        }
    
        // apply category filters
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $qb = ModUtil::apiFunc('VerySimpleLinkCollection', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $objectType, 'catids' => $vars['catIds']));
            }
        }
        ...
        if (empty($blockinfo['title'])) {
            $blockinfo['title'] = $this->__('VerySimpleLinkCollection items');
        }
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('VerySimpleLinkCollection', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $vars['objectType'] = $this->request->request->filter('objecttype', 'link', FILTER_SANITIZE_STRING);
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('VerySimpleLinkCollection', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
