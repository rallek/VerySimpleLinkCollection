 ...
 */
class VerySimpleLinkCollection_Util_Base_Workflow extends Zikula_AbstractBase
{
    /**
    ...
    public function getActionsForObject($entity)
    {
        ...
        $listHelper = new VerySimpleLinkCollection_Util_ListEntries($this->serviceManager);
    ...
    {
        $amounts = array();
        $modname = 'VerySimpleLinkCollection';
        ...
        if (SecurityUtil::checkPermission($modname . ':' . ucwords($objectType) . ':', '::', ACCESS_ADD)) {
            ...
            if ($amount > 0) {
                $amounts[] = array(
                    'aggregateType' => 'linksApproval',
                    'description' => $this->__('Links pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->_fn('One link is waiting for approval.', '%s links are waiting for approval.', $amount, array($amount))
                );
            }
        }
