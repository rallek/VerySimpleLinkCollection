 ...
 */
class VerySimpleLinkCollection_Form_Plugin_Base_ItemSelector extends Zikula_Form_Plugin_TextInput
{
    /**
    ...
    public function render(Zikula_Form_View $view)
    {
        ...
        if ($firstTime) {
            ...
            PageUtil::addVar('javascript', 'modules/VerySimpleLinkCollection/javascript/VerySimpleLinkCollection_finder.js');
            PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('VerySimpleLinkCollection'));
        }
        $firstTime = false;

        if (!SecurityUtil::checkPermission('VerySimpleLinkCollection:' . ucwords($this->objectType) . ':', '::', ACCESS_COMMENT)) {
            return false;
        }

        $categorisableObjectTypes = array('link');
        $catIds = array();
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            ...
            $catIds = ModUtil::apiFunc('VerySimpleLinkCollection', 'category', 'retrieveCategoriesFromRequest', array('ot' => $this->objectType));
        }
        ...
        $properties = null;
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('VerySimpleLinkCollection', 'category', 'getAllProperties', array('ot' => $this->objectType));
        }
    ...
    {
        parent::decode($view);
        $this->objectType = FormUtil::getPassedValue('VerySimpleLinkCollection_objecttype', 'link', 'POST');
        $this->selectedItemId = $this->text;
    }
}
