...
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="verySimpleLinkCollectionLinkQuickNavForm" class="verysimplelinkcollection-quicknav">
    <fieldset>
        ...
        {if !isset($categoryFilter) || $categoryFilter eq true}
            {modapifunc modname='VerySimpleLinkCollection' type='category' func='getAllProperties' ot=$objectType assign='properties'}
            {if $properties ne null && is_array($properties)}
                ...
                {foreach key='propertyName' item='propertyId' from=$properties}
                    ...
                    {/if}
                        ...
                        &nbsp;
                        {selector_category name="`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIdList.$propertyName categoryRegistryModule='VerySimpleLinkCollection' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
                {/foreach}
                {/nocache}
            {/if}
        {/if}
